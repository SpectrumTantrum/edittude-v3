...
def assemble(edl: EditDecision, out: Path, work_dir: Path) -> Path:
    ...
    for index, event in enumerate(edl.events):
        ...
        _encode_event(event, dest, width, height, edl.fit)
...
def _encode_event(event: Event, dest: Path, width: int, height: int, fit: str = "pad") -> None:
    src = Path(event.src)
    if not src.is_file():
    ...
    vf = scale_filter(width, height, fit=fit, zoom=event.zoom, cx=event.cx, cy=event.cy)
    args = [
        "-ss",
        f"{event.in_point:.3f}",
        "-i",
